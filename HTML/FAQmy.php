...
<Body>
<br><br>
<center><h1 class="Ttxtstyle">My FAQs</h1></center>
<section>
	<center>
	<?php
	
	global $con;
	$sql ="SELECT * FROM faq Where CID='$CID'";
	...
	while ($row = $result->fetch_assoc()) {
		...
		echo"<div id='CareersBody'>
			<div style='display:flex; width:60%; justify-content:space-between; background-color:white;padding:20px 20px;'>
							<div style='margin-top:30px;'><h2>FAQ ID : $FAQID</h2></div>
							<div style='margin-top:30px;'><h2>Subject : $Subject</h2></div>";
							
					echo"<div class='btndiv'><a href='FAQUpdate.php?FID=$FAQID'> <button name='edit' value='Edit' id='' type='button' class='Btn4'>  Edit </button></a><a name='Delete' href='FAQDelete.php?FID=$FAQID'> <button name='delete' value='Delete' id='' type='button' class='Btn4'>  Delete </button></a>";
					echo"</div>";
			echo"</div>";
		echo"</div>";
		}
