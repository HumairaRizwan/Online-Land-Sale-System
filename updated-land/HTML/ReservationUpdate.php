...
<section style="width=100%;" class="logingbody">
	<div id="formStyle3">
		<center>
		<form action="UpdatePrecessReserveaLand.php?GetReID=<?php echo $RID;?>" method="POST" >
			<fieldset>
				<?php
                	$sql = "SELECT * FROM reservation WHERE RID=$R_ID";
					...
					while ($row = $result->fetch_assoc())
				   {
						...
						$RTEL = $row['RTEL'];
						$REmail = $row['REmail'];
						$RMassage = $row['RMassage'];
						$CID = $row['CID'];
				   }?>
			<div class="col-50">
					<label for="name">Name</label>
					<input name='Rname'type="text" id="name" value="<?php echo $RName; ?>" required>

					<label for="Telephone">Telephone</label>
					<input name='Rtel'type="text" id="Telephone" value="<?php echo $RTEL; ?>" required>

					<label for="email">Email</label>
					<input name='Remail'type="text" value="<?php echo $REmail; ?>" id="email" required>		

					
					<label for="Message">Message</label>
					<input name='Rmassage'type="text" value="<?php echo $RMassage; ?>" id="Message" required>
					<br>
					<button name="RUpdate" value="update" id="" type="submit" class="Btn1" >Update</button>
